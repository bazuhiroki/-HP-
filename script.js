...
// --- API通信用の関数 ---
// 1. Notionから映画リスト（IDとタイトルとカバー画像）を取得
...
gridItems.forEach(item => {
    item.addEventListener('click', async (event) => {
        if (event.target.closest('.watched-button')) return; // ボタンクリック時はフォーカス処理を中断
        ...
        if (item.classList.contains('is-focused')) {
            ...
            if (item.classList.contains('movie')) {
                ...
                detailView.innerHTML = `<p style="color: #888;">Notionから情報を取得中...</p>`;

                const notionMovies = await fetchNotionMovies();
                
                // 複数の映画を表示するためにHTMLを生成
                let moviesHtml = '';
                for (const movie of notionMovies) {
                ...
                detailView.innerHTML = moviesHtml || '<p>Notionに映画データがありません。</p>';
                
                // 「視聴済みにする」ボタンにイベントリスナーを追加
